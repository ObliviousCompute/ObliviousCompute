...
Each piece is defined by its own invariants. Every observer evaluates the proposed transitions according to those same invariants. A pawn moves according to one set of constraints. A queen moves according to another. These are not preferences. They are the rules that define each piece.
